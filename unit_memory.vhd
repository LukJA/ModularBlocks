...
begin
	...
	for k in 0 to mem_size-1 generate
      REGX : work.DFF_Rising generic map (word_width) port map (regClock(k) and ControlIn(0) , regDs(k), regQs(k));
...
entity U_mem_page2p is
	...
	port(	
		...
		AddressIn	: in array_t(1 downto 0)(addr_width-1 downto 0) := (others=>'0');
		ControlIn 	: in vector_t(1 downto 0) := (others=>'0');
		Enable		: in bit_t := '0';
		
		DataOut 		: out array_t(1 downto 0)(word_width-1 downto 0) := (others=> (others=>'0'));
		AddressOut	: out array_t(1 downto 0)(addr_width-1 downto 0) := (others=>'0');: 
		ControlOut 	: out bit_t;
		Reset			: in  bit_t := '0'
		);
...
begin
	...
	/* registers */
	for k in 0 to mem_size-1 generate
      REGX : work.DFF_Rising generic map (word_width) port map ((regClockB(k) and ControlIn(0)) or (regClockA(k) and ControlIn(1)) , regDs(k), regQs(k));
   end generate GEN_REG;
	
	---- PORT A ----
	/* clock one hot encoder */ 
	clk_mux: entity work.F_onehot_enc generic map (addr_width, mem_size) port map (AddressIn, regClockA);	
	/* input demultiplexer */
	in_mux: entity work.F_demux_array generic map (mem_size, word_width, addr_width) port map (DataIn(1), AddressIn, regDA);
	/* output multiplexer */
	out_mux: entity work.F_mux_array generic map (mem_size, word_width, addr_width) port map (regQs, AddressIn, DataOut(1));
	
		---- PORT B ----
	/* clock one hot encoder */ 
	clk_mux: entity work.F_onehot_enc generic map (addr_width, mem_size) port map (AddressIn, regClock);	
	/* input demultiplexer */
	in_mux: entity work.F_demux_array generic map (mem_size, word_width, addr_width) port map (DataIn(0), AddressIn, regDB);
	/* output multiplexer */
	out_mux: entity work.F_mux_array generic map (mem_size, word_width, addr_width) port map (regQs, AddressIn, DataOut(0));
